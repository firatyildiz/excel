...
*--------------------------------------------------------------------*
* ToDos:
*        2doÂ§1   Support partial formatting of strings in cells
*--------------------------------------------------------------------*
...
* Â§1  Parse shared strings file and get into internal table
*   So far I have encountered 2 ways how a string can be represented in the shared strings file
*   Â§1.1 - "simple" strings
*   Â§1.2 - rich text formatted strings
...
*       as long as rich text formatting is not supported (2doÂ§1) ignore all info about formatting
*                Example:  see above
*--------------------------------------------------------------------*
...
*              ...
*              parts with various formatinformation - see Â§1,Â§2,Â§3,Â§4
*              ...
*          <cellXfs count="26">
...
*--------------------------------------------------------------------*
  lt_num_formats   = load_style_num_formats( lo_styles_xml ).   " Â§1
  lt_fills         = load_style_fills( lo_styles_xml ).         " Â§2
  lt_borders       = load_style_borders( lo_styles_xml ).       " Â§3
  lt_fonts         = load_style_fonts( lo_styles_xml ).         " Â§4
...
* Â§1    A cell has 4 outer borders and 2 diagonal "borders"
...
* Â§2    and read the corresponding formatting information
...
*--------------------------------------------------------------------*
* Â§1 Determine what kind of border we are talking about
*--------------------------------------------------------------------*
...
*--------------------------------------------------------------------*
* Â§2 Read the border-formatting
*--------------------------------------------------------------------*
...
*--------------------------------------------------------------------*
* ToDos:
*        2doÂ§1   Explain gaps in predefined formats
*--------------------------------------------------------------------*
...
* Â§1 built-in numberformats
* Â§2 and those that have been explicitly added by the createor of the excel-file
*--------------------------------------------------------------------*
...
*--------------------------------------------------------------------*
* Â§2   Get non-internal numberformats that are found in the file explicitly
...
*--------------------------------------------------------------------*
* ToDos:
*        2doÂ§1   Move macro-reading from zcl_excel_reader_xlsm to this class
...
* Â§1  Get the position of files related to this workbook
...
*        </Relationships>
*
* Â§2  Load data that is relevant to the complete workbook
*     Currently supported is:
*   Â§2.1    Shared strings  - This holds all strings that are used in all worksheets
*   Â§2.2    Styles          - This holds all styles that are used in all worksheets
*   Â§2.3    Worksheets      - For each worksheet in the workbook one entry appears here to point to the file that holds the content of this worksheet
*   Â§2.4    [Themes]                - not supported
*   Â§2.5    [VBA (Macro)]           - supported in class zcl_excel_reader_xlsm but should be moved here and autodetect
*   ...
*
* Â§3  Some information is held in the workbookfile as well
*   Â§3.1    Names and order of of worksheets
*   Â§3.2    Active worksheet
*   Â§3.3    Defined names
*   ...
*     Following is an example how this file could be set up
...
*--------------------------------------------------------------------*
*   Â§2.2    Styles           - This holds the styles that are used in all worksheets
*--------------------------------------------------------------------*
      WHEN lcv_styles.
        CONCATENATE lv_path ls_relationship-target
            INTO lv_full_filename.
...
*--------------------------------------------------------------------*
* Â§3  Some information held in the workbookfile
*--------------------------------------------------------------------*
...
*--------------------------------------------------------------------*
* Â§2  We need to read the the file "\\_rels\.rels" because it tells
...
*--------------------------------------------------------------------*
* ToDos:
*        2doÂ§1   Determine whether the replacement should be done
*                iterative to allow /../../..   or something alike
*        2doÂ§2   Determine whether /./ has to be supported as well
*        2doÂ§3   Create unit-test for this method
*
*                Please don't just delete these ToDos if they are not
*                needed but leave a comment that states this
*--------------------------------------------------------------------*
...
*--------------------------------------------------------------------*
* Â§1  This routine will receive a path, that may have a relative pathname (/../) included somewhere
...
*--------------------------------------------------------------------*
* ToDos:
*        2doÂ§1   Map Document Properties to ZCL_EXCEL
*--------------------------------------------------------------------*
...
*--------------------------------------------------------------------*
* Â§1  Create EXCEL-Object we want to return to caller
...
* Â§3  Extracting from this the path&file where the workbook is located
*     Following is an example how this file could be set up
*        <?xml version="1.0" encoding="UTF-8" standalone="true"?>
*        <Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
*            <Relationship Target="docProps/app.xml" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/extended-properties" Id="rId3"/>
...
*--------------------------------------------------------------------*
* Â§2  Get file in folderstructure
*--------------------------------------------------------------------*
  lo_rels = get_ixml_from_zip_archive( '_rels/.rels' ).

*--------------------------------------------------------------------*
* Â§3  Cycle through the Relationship Tags and use the ones we need
*--------------------------------------------------------------------*
...
ENDMETHOD.
  ...
  METHOD zif_excel_reader~load_file_from_mime.
    ...
    IF sy-subrc EQ 0.
      LOOP AT lt_mime INTO ls_mime.
        ...
        IF <lfs_data> IS ASSIGNED.
          CONCATENATE lv_excel_data <lfs_data> INTO lv_excel_data IN BYTE MODE.
          UNASSIGN <lfs_data>.
        ENDIF.
      ENDLOOP.
